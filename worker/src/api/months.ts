 ...
 *
 * There is no subscriber series. It was read from `channel_snapshot`, which
 * is kept for 30 days only (#223), so it could never cover more than the last
 * month or two. The statistics page draws subscribers from the published
 * milestones instead (#225).
 */
...
/** The series, summed the same way: null before a member exists counts as nothing, and 0 after is a real answer. */
...
/** One channel's row in the response: the seven counted series. */
type ChannelSeries = { channelId: string } & Record<SeriesName, (number | null)[]>;

/** The `total` object's shape: the seven series summed. */
type Total = Record<SeriesName, number[]>;
...
function emptyResponse(): MonthsResponse {
  return {
    ...
    months: [],
    channels: [],
    total: Object.fromEntries(SERIES.map((name) => [name, [] as number[]])) as Total,
  };
}

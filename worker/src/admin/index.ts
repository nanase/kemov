...
/**
 * A path segment as a footprints_event, genet_tune or genet_person id, or
 * null when it is not a plain positive integer. `Number.isSafeInteger`
 * guards against a segment with enough digits to round to a different
 * integer, or to `Infinity`, once `Number` parses it - `/^[1-9]\d*$/` alone
 * only rules out a non-digit shape.
 */

...
/**
 * `/members/<channel id>`: a permalink to one member (#137), so that sharing
 * it carries that member's name rather than the site's own title.
 *
 * The page does not exist yet - #144's later work adds it - so this rewrites
 * whatever `ASSETS` already serves at `/members/` rather than building a page
 * of its own. The worker only ever sees this request because no built file
 * answers `/members/<id>` exactly, the same reason `/api/*` reaches
 * `../api/index.ts`: Cloudflare serves a matching file directly and wakes the
 * worker only for the paths that are not one.
 */
 ...
 * including `/members/` itself, which has no id to fill in and is left to
 * `ASSETS` untouched.
 */
function matchRoute(pathname: string): Route | null {
  ...
  if (resource === 'members') return { kind: resource, id };
 ...
 * D1.
 *
 * The stored name is unescaped before use. Some rows carry `&quot;`, `&amp;`
 * and `&#39;` as literal text rather than the characters they stand for -
 * src/components/genet/MarkDown.vue already unescapes the same way before
 * showing a name on screen, and this reuses that exact function rather than a
 * second copy of the same rule. `HTMLRewriter`'s `setInnerContent`/
 * `setAttribute` (see `rewrite` below) escape whatever string they are
 * handed, so unescaping first is what keeps that the only escaping the text
 * goes through - skipping it would show `&amp;quot;` instead of a literal
 * `"`.
 */
...
/**
 * `/members/<id>`, or null for a path that does not name one -
 ...
 * hand this a page `/members/` does not have yet: it is added in a later PR,
 * and until then `env.ASSETS` answers every request under it with a 404,
 * which is the "ASSETS からページが取れない" path below rather than the
 * tested rewrite.
 */

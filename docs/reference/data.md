...
そのため 30 日の増減に限り、「最新 − 30 日」以前に行が無ければ、それより後で最も古い行と比べます。比べる間隔は 30 日より短くなり、短さは最長で約 1 時間 20 分です。内訳は、毎時の回の間隔の 1 時間と、両端の行が 10 分ごとの回の刻みでずれる分です。増減の許容の幅は 30 日の前後 3 日なので（`worker/src/lib/delta.ts`）、その中に収まります。許容の幅の外にしか行が無いときは、これまでどおり「この期間の記録がまだありません」になります。
...
ただし `video` のファイルには、取れなくなった動画のうち、最後に取れてから 2 日を過ぎたものを入れません。その行の値は最後に取れた時点のもので、ファイルは R2 に最長 28 日残るため、2 日を過ぎた行を入れると 30 日を超えてしまいます（[保持期間](#保持期間)）。そのため、バックアップから戻しても、取れなくなって 2 日を過ぎた動画は戻りません。どれも公開サイトの集計には入らない行です。集計は `availability` が `public` の動画だけを数えます。
...
- `video/` の取れる動画は、video-update がすべてを順に読み直す。1 巡は最長で「動画の数 ÷ ((50 − 20) × 144)」日で、2026-09-25 の 6,460 本なら約 1.5 日

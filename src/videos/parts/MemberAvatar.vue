...
/**
 * One channel's picture, with something to show when it does not arrive.
 *
 ...
 * know in advance and has to be able to draw a channel without their picture.
 *
 * What it draws instead is the channel's own colour and the first character
 * of their name, at the same size and full strength. Kept identical to
 * `/stats/`'s `src/stats/parts/MemberAvatar.vue` (#161, not yet on main) so
 * the two can be pointed at one shared copy once it merges.
 */
const { src, name, color, size, dark } = defineProps<{
  src: string | null;
  name: string;
  /** The channel's own colour, or null for a stand-in with no channel behind it. */
  color: string | null;
  size: number;
  dark: boolean;
}>();

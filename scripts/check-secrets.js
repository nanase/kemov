 ...
 *
 * Usage: node scripts/check-secrets.js <secret-list.json>
 *
 ...
 * cost. This is the check nobody was doing.
 *
...
if (listPath === undefined) {
  console.error('usage: node scripts/check-secrets.js <secret-list.json>');
  process.exit(2);
}
...
try {
  ...
  if (missing.length > 0) {
    ...
    console.error('set each one with `bun wrangler secret put <name>`; see docs/guides/deployment.md.');
    process.exitCode = 1;
  } else {
